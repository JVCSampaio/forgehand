...
def dashboard_snapshot(config: ForgehandConfig, *, limit: int = 100) -> dict[str, Any]:
    ...
    totals = {"tasks": 0, "tokens": 0, "calls": 0, "wall_seconds": 0.0}
    for task in tasks:
...
<section class="cards"><div class="card"><div class="label">Tasks</div><div id="tasks" class="value">—</div></div><div class="card"><div class="label">Worker tokens</div><div id="tokens" class="value">—</div></div><div class="card"><div class="label">Model calls</div><div id="calls" class="value">—</div></div><div class="card"><div class="label">Success rate</div><div id="rate" class="value">—</div></div></section>
<div class="table-wrap"><table><thead><tr><th>Status</th><th>Repository</th><th>Summary</th><th>Tokens</th><th>Calls</th><th>Changed</th><th>Finished</th></tr></thead><tbody id="rows"></tbody></table></div>
<footer id="notice"></footer></main><script>
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
async function refresh(){const d=await fetch('/api/summary',{cache:'no-store'}).then(r=>r.json());const t=d.totals;document.querySelector('#tasks').textContent=t.tasks.toLocaleString();document.querySelector('#tokens').textContent=t.tokens.toLocaleString();document.querySelector('#calls').textContent=t.calls.toLocaleString();document.querySelector('#rate').textContent=t.tasks?Math.round(100*(d.statuses.success||0)/t.tasks)+'%':'—';document.querySelector('#notice').textContent=d.notice;document.querySelector('#rows').innerHTML=d.tasks.map(x=>`<tr><td class="status ${esc(x.status)}">${esc(x.status)}</td><td><code>${esc(x.repository)}</code></td><td>${esc(x.summary||'—')}</td><td>${x.tokens.toLocaleString()}</td><td>${x.calls}</td><td>${x.changed_files}</td><td>${esc(x.finished_at||'—')}</td></tr>`).join('')||'<tr><td colspan="7">No completed tasks yet.</td></tr>'}refresh();setInterval(refresh,5000);
</script></body></html>"""
